  ...
  const handleAdvanceSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.qualityCheckPassed && scrap > 0 && validQty > 0) {
        ...
        showToast(`${validQty} items sent to Assembly, ${scrap} items sent to QC for inspection.`, 'success');
      } else {
        ...
        {activeTab === 'history' ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {batches.map((batch, idx) => (
              <div key={batch.id || idx} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/20 hover:border-indigo-200 transition-all group flex flex-col relative overflow-hidden">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-1 relative z-0">{batch.manufacturingAttributes?.batchNumber || batch.batchNumber || batch.workOrderNumber || `BATCH-${batch.id}`}</h3>
                <p className="text-sm font-semibold text-slate-500 mb-2 relative z-0">{batch.manufacturingAttributes?.itemName || batch.itemName || 'Raw Material Item'}</p>
            ...
            <form onSubmit={handleCreateSubmit} className="p-8 space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 px-6 py-3 border border-slate-200 rounded-xl text-slate-600 font-medium hover:bg-slate-50 transition-all uppercase tracking-wider text-sm"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  disabled={loading || !createFormData.productId}
                  className="flex-[1.5] px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <>
