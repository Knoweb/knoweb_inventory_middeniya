...
const INIT_PO = {
  ...
  currency: 'LKR',
  notes: '',
  items: [{ itemCode: '', productName: '', quantity: '', unitPrice: '' }],
};

// Removed INIT_SO as Sales Orders are not needed here
...
function CreatePurchaseOrderModal({ suppliers, onClose, onCreated }) {
  ...
  const handleProductSelect = (idx, value) => {
    ...
    setForm(prev => {
      ...
      items[idx] = {
        ...items[idx],
        itemCode: product ? (product.itemCode || product.sku || `RAW-PLAST-${product.id}`) : '', // Adapt as per backend actual fields
        productName: product ? (product.name || product.itemName) : '',
        productId: product ? product.id : '' // Keeping id just for the UI select binding
      };
      return { ...prev, items };
    });
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-y-auto animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">Expected Delivery Date *</label>
              <input
                type="date"
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all"
